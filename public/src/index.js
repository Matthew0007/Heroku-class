...
async function deleteProduct() {
  ...
  // url for api call
  const url = `${api.BASE_URL}/product/${this.id}`
...
async function addOrUpdateProduct() {
  ...
  const formProduct = getProductForm();
  // log to console
  console.log('%cNew Product: ', 'color: green');
  ...
  const rows = products.map((product) => {
    ...
    let row = `<tr>
                ...
                <td><button data-product_id="${product._id}" 
                  ...
                  <button data-product_id="${product._id}" id="${product._id}" 
                  class="btn btn-sm btn-outline-danger btn-delete-product deleteMe" 
                   >
                  <span class="bi bi-pencil-square" 
